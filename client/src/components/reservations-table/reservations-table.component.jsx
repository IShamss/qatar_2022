...
useEffect(()=>{
  instance.get("/reservations/"+currentUser._id).then(res=>{
    ...
    instance.get("/matches/"
            ...
            for (let i = 0; i < response.data.matches.length; i++) {
              obj[response.data.matches[i]._id] = response.data.matches[i].name;
            }
            setMatches_M(obj);
		  }).catch((err)=>{
		console.log(err)	
  ...
  const handleDelete=(resId)=>{
    // console.log(resId)
    // console.log(e);
    instance.delete("/reservation/"+resId).then(res=>console.log("deleted")).catch(err=>console.log(err))
  }
          ...
          {reservations.map((row) => (
            ...
            >
              <TableCell component="th" scope="row">
                {`${teams_M[row.team1]} vs ${teams_M[row.team2]}`}
              </TableCell>
              {/* <TableCell align="right">{`${matches}`}</TableCell> */}
              <TableCell align="right"><IconButton onClick={()=>handleDelete(row._id)}><DeleteOutlineIcon /></IconButton></TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
