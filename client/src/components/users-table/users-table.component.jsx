...
export default function UsersTable() {
    ...
    const [users_M, setUsers_M] = React.useState([]);
    ...
    useEffect(() => {
        ...
        for (let u of users) {
            instance
                .get("/reservations_No/" + u._id)
                .then((response) => {
                    nums[u._id] = response.data.reservaions_No;
                    setUsers_M(nums);
                    setUp((prev) => {
                        return prev + 1;
                    });
                ...
                <TableBody>
                    {users.map((user) => (
                        <StyledTableRow key={user.user_name}>
                            ...
                            <StyledTableCell align="center">
                                {up ? users_M[user._id] : users_M[user._id]}
                            </StyledTableCell>
                            <StyledTableCell align="center">
                                {!user.to_be_a_manager ? (
                                    getType(user.role)
                                ) : (
                                    <React.Fragment>
                                        <Button
